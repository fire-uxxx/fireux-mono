<template>
  <div class="profile-page">
    ...
    <!-- Employer Profile Status Section -->
    <div class="profile-status-section">
      <div
        v-if="hasEmployerProfile"
        class="profile-status-card profile-status-active"
      >
        <div class="profile-status-content">
          <h2 class="profile-status-title">✅ You have an Employer profile</h2>
          <p class="profile-status-description">
            Your company profile is active and visible to potential employees.
          </p>
          <div class="profile-status-actions">
            <NuxtLink to="/dashboard/employer-profile" class="btn btn-primary">
              View Your Profile
            </NuxtLink>
            <NuxtLink
      ...
      <div v-else class="profile-status-card profile-status-inactive">
        <div class="profile-status-content">
          <h2 class="profile-status-title">🏢 Create your Employer profile</h2>
          <p class="profile-status-description">
            Set up your company profile to post jobs and attract talented
            professionals.
          </p>
          <div class="profile-status-actions">
            <NuxtLink to="/dashboard/employer-profile" class="btn btn-primary">
...
// Get current user and check if they have an employer profile
const { appUser } = await useAppUser()
const hasEmployerProfile = computed(() => {
  if (!appUser.value) return false
  return (
    appUser.value.profiles?.some(
      (profile) => profile.type === 'employer' && profile.is_active
    ) || false
  )
})
...
const copyData = async () => {
  const combinedData = {
    ...
    summary: {
      totalEmployers: employers.value?.length || 0,
      totalAppUsers: appusers.value?.length || 0,
      employersWithAppUsers:
        employers.value?.filter((employer) =>
          appusers.value?.find((user) => user.uid === employer.uid)
        ).length || 0,
    },
  }
