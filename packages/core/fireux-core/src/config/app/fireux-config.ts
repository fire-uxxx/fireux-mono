...
/**
 * Minimal, canonical FireUX Nuxt config builder.
 * - Always includes `fireux-core` unless includeCore === false
 * - Merges additional `modules` w/ de-dupe
 * - Exposes tenant metadata via runtimeConfig.public
 */
export function createFireuxConfig(opts: TenantConfig): NuxtConfig {
  ...
  return {
    // Keep srcDir to support current app/pages layout in apps
    srcDir: 'app/',
    dir: { public: '../public' },
    modules,
    app: {
      head: { title: opts.appName },
    },
    runtimeConfig: {
      public: {
